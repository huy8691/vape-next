...
import { schema, schemaDelay, schemaUpdate } from './validations'
...
const DiscountComponent = (props: Prop) => {
  ...
  useEffect(() => {
    if (router.query.id) {
      getListDiscountOfDC(Number(props.merchantId))
        .then((res) => {
          const { data } = res
      ...
      getListDCForApplytoDiscount(Number(props.merchantId))
        .then((res) => {
          const { data } = res.data
  ...
  useEffect(() => {
    getDelayPayment(props.merchantId)
      .then((res) => {
        const { data } = res.data
  ...
  const {
    ...
    resolver: yupResolver(schemaDelay(t)),
    mode: 'all',
  })
  ...
  const onSubmitDelay = (value: SetDelayPaymentType) => {
    ...
    setDelayPayment(props.merchantId, submitValue)
      .then(() => {
        getDelayPayment(props.merchantId)
          .then((res) => {
            const { data } = res.data
            setValueDelay('duration', data.duration)
            pushMessage(t('message.setDelayPaymentSuccessfully'), 'success')
          })
          .catch(({ response }) => {
            const { status, data } = response
            pushMessage(handlerGetErrMessage(status, data), 'error')
  ...
  const onSubmitUpdateRule = (value: SubmitAddRuleType) => {
    ...
    if (stateRadio === 2 && stateDetailDiscount?.is_apply_all_channel) {
      pushMessage(
        t('message.thereIsAlreadyADiscountRuleThatAppliedToChannel'),
        'error'
      )
      return
    }
    ...
    <>
      <Box>
        ...
        <Box>
          ...
          >
            {t('details.delayPaymentRule')}
          </Typography>
          <form onSubmit={handleSubmitDelay(onSubmitDelay)}>
              ...
              <Controller
                ...
                render={({ field }) => (
                  <>
                    <InputLabelCustom>
                      {' '}
                      {t('details.delayDuration')}
                    </InputLabelCustom>
                    <FormControl>
                      <TextFieldCustom
                        defaultValue={0}
                        placeholder={t('details.enterNumberOfDays')}
                        error={!!errorsDelay.duration}
                        {...field}
                      />
                      <FormHelperText error>
                        {errorsDelay.duration &&
                          `${errorsDelay.duration.message}`}
                      </FormHelperText>
                    </FormControl>
                  </>
                )}
              ...
              <Stack spacing={2}>
                ...
                <Stack direction="row" spacing={2}>
                  <ButtonCustom
                    variant="outlined"
                    size="large"
                    onClick={() => setStateDrawerUpdate(false)}
                  >
                    {t('details.cancel')}
                  </ButtonCustom>
                  <ButtonCustom variant="contained" type="submit" size="large">
