...
import {
  ...
  updateAddressBook,
  updateExternalOrder,
} from './externalOrderUpdateAPI'
import { AddressDataType, Item } from './externalOrderUpdateModel'
import {
  schemaAddress,
...
const UpdateExternalOrderComponent: React.FC = () => {
  ...
  useEffect(() => {
    if (router.query.id) {
      dispatch(loadingActions.doLoading())
      getDetailExternalOrder(router.query?.id as string)
        .then((res) => {
          const { data } = res.data

          if (data?.code.indexOf('TWSEO') !== -1) {
            setValue(
              'code',
              data?.code?.split(
                data?.code?.slice(0, data?.code.indexOf('E') + 1)
              )[1] || data?.code
            )
          } else {
          ...
          setValue('discount_amount', Number(data.discount.toFixed(2)))
          setValue('tax_amount', Number(data.tax_amount.toFixed(2)))
          ...
          setStateProductSelect(data.items)
  ...
  const onSubmitValue = (values: any) => {
    ...
    try {
      updateExternalOrder(
        ...
        .then(() => {
          dispatch(loadingActions.doLoadingSuccess())
          pushMessage(t('message.updateExternalOrderSuccessfully'), 'success')
          router.push(
...
export default UpdateExternalOrderComponent
