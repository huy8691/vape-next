...
import {
  ...
  getListAddress,
  updateAddressBook,
} from './externalOrderCreateOCRAPI'
import { AddressDataType, Item } from './externalOrderCreateOCRModel'
...
import DrawerAddProductInventory from './part/DrawerAddProductOCR'
...
const CreateOCRExternalOrderComponent: React.FC = () => {
  ...
  const [stateProductSelect, setStateProductSelect] = useState<any[]>([])
  ...
  const [dataOCR, setDataOCR] = useState<any>(null)
  ...
  const {
    ...
    resolver: yupResolver(schemaExternalSupplier),
    mode: 'all',
  })
  ...
  const handleConfirmAddress = () => {
    for (const i in stateListAddress?.data) {
      if (stateListAddress?.data[Number(i)].id == stateSelectedValue) {
        ...
        setValue('address', stateListAddress?.data[Number(i)].address)
        setValue('address_name', stateListAddress?.data[Number(i)].name)
        setValue('phone_number', stateListAddress?.data[Number(i)].phone_number)
      }
    }
    reset({
      name: '',
    ...
    setStateOpenDialogAddress(false)
    pushMessage(t('changeAddressSuccessfully'), 'success')
  }
  ...
  useEffect(() => {
    if (Object.keys(router.query).length !== 0) {
      const data = {
        ...router.query,
        table: JSON.parse(router.query.table as string),
      }
      setDataOCR({
        ...data,
        table: data.table.map((item: any) => {
          return {
            ...
            price: isNaN(item.Price) ? 0 : Number(item.Price).toFixed(2),
            quantity: item.Quantity || 0,
            products: item.products,
          }
        }),
      })
      setStateProductSelect(data.table)
      setValue(
        'items',
  ...
  const onSubmitValue = (values: any) => {
    setDataOCR((prev: any) => ({
      ...
      table: values.items.map((item: any, index: number) => {
        return {
          ...prev.table[index],
          price: isNaN(item.price) ? 0 : Number(item.price).toFixed(2),
          quantity: item.quantity || 0,
  ...
  const totalAmount = useMemo(() => {
    return (
      totalPrice +
      Number(watch('tax_amount')) -
      Number(watch('discount_amount'))
    )
  ...
  const onSubmitCreateExternalSupplier = (values: any) => {
    ...
    try {
      createExternalSupplier(
        ...
        .then(() => {
          pushMessage(t('updateExternalSupplierSuccessfully'), 'success')
      ...
      pushMessage(t('someThingsGoneWrong'), 'error')
      dispatch(loadingActions.doLoadingFailure())
    }
  }
  ...
  return (
    <>
                    ...
                    <>
                      ...
                      >
                        ...
                        >
                          ...
                          >
                            {stateProductSelect[index]?.Description || 'N/A'}
                          </Typography>
                          <Stack
                            direction={'row'}
                            alignItems={'flex-end'}
                    ...
                    <LocalizationProvider dateAdapter={AdapterMoment}>
                      <DatePicker
                        ...
                        onChange={(newValue: Moment | null) => {
                          setValue('order_date', newValue as any)
                        }}
              ...
              render={({ field }) => (
                <Box>
                  ...
                  <FormControl fullWidth>
                    <div className={classes['input-number']}>
                      <CurrencyNumberFormat
                        defaultPrice={Number(field.value).toFixed(2)}
                        propValue={(value) => {
      ...
      <DrawerAddProductInventory
        dataOCR={dataOCR}
        open={openDrawerAddProduct}
        onClose={setOpenDrawerAddProduct}
      />
    </>
  )
}

export default CreateOCRExternalOrderComponent
