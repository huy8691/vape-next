...
const ListBarChartAverageNumberOfUnitsPerTransaction: React.FC<Props> = (
  props
) => {
  ...
  const [stateListData, setStateListData] = React.useState<
    {
      store: string
      products: { id: number; name: string; average: number }[]
      all_store: boolean
    }[]
  >([])
        ...
        {stateListData.map((item, index) => {
          return (
            <Grid xs={4} key={index}>
              ...
              <Chart
                // options={item?.products?.map((i) => i.name)}
                type="bar"
                series={[
                  {
                    name: 'Average',
                    data: item?.products?.map((i) => i.average),
                  },
                ]}
                options={{
                  xaxis: {
                    categories: item?.products?.map((i) => i.name),
                  },
...
export default ListBarChartAverageNumberOfUnitsPerTransaction
