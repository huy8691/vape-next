...
const ListPieChartCategoryProduct: React.FC<Props> = (props) => {
  ...
  const [stateListData, setStateListData] = React.useState<
    {
      store: string
      categories: { id: number; name: string; profit_margin: number }[]
      all_store: boolean
    }[]
  >([])
        ...
        {stateListData.map((item, index) => {
          return (
            <Grid xs={4} key={index}>
              ...
              <Chart
                series={item?.categories?.map((i) => i.profit_margin)}
                options={{
...
export default ListPieChartCategoryProduct
