...
const ListPieChartEmployees: React.FC<Props> = (props) => {
        ...
        {stateListData.map((item, index) => {
          return (
            <Grid xs={4} key={index}>
              ...
              <Chart
                series={item?.employees?.map((i) => i.sold_products)}
                options={{
                  labels: item?.employees?.map(
...
export default ListPieChartEmployees
