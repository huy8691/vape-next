...
const ListPieChartProducts: React.FC<Props> = (props) => {
        ...
        {stateListData.map((item, index) => {
          return (
            <Grid xs={4} key={index}>
              ...
              <Chart
                series={item?.products?.map((i) => i.quantity)}
                options={{
                  labels: item?.products?.map((i) => i.name),
                  legend: {
                    show: true,
                    position: 'bottom',
                  },
...
export default ListPieChartProducts
