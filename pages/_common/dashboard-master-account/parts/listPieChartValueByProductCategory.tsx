...
const ListPieChartValueByProductCategory: React.FC<Props> = (props) => {
  ...
  const [stateListData, setStateListData] = React.useState<
    {
      store: string
      categories: { id: number; name: string; values: number }[]
      all_store: boolean
    }[]
  >([])
        ...
        {stateListData.map((item, index) => {
          return (
            <Grid xs={4} key={index}>
              ...
              <Chart
                series={item?.categories?.map((i) => i.values)}
                options={{
                  labels: item?.categories?.map((i) => i.name),
                  legend: {
                    show: true,
                    position: 'bottom',
                  },
                  ...
                  tooltip: {
                    y: {
                      formatter: function (value, opts) {
                        console.log('opts', value)
                        const val = opts.globals.series[opts.seriesIndex]
                        return (
                          Number(
                            opts.globals.seriesPercent[opts.seriesIndex]
                          ).toFixed(2) +
                          '%' +
...
export default ListPieChartValueByProductCategory
