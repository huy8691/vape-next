...
const ListPieChartStore: React.FC<Props> = (props) => {
  ...
  const [stateTypeDay, setStateTypeDay] = React.useState<
    'day' | 'year' | 'month'
  >('month')
  const [stateListTopData, setStateListTopData] = React.useState<
    {
      store: string
      values: number
      all_store: boolean
  ...
  const [stateListBottomData, setStateListBottomData] = React.useState<
    {
      store: string
      values: number
      all_store: boolean
  ...
  const handleGetTopStoreSelling = () => {
    dispatch(loadingActions.doLoading())
    callAPIWithToken({
      url: 'api/report/master/top-store-selling',
      method: 'get',
      params: {
  ...
  const handleGetBottomStoreSelling = () => {
    dispatch(loadingActions.doLoading())
    callAPIWithToken({
      url: 'api/report/master/bottom-store-selling',
      method: 'get',
      params: {
        fromDate: stateDateRange.fromDate.format('YYYY-MM-DD'),
        toDate: stateDateRange.toDate.format('YYYY-MM-DD'),
  ...
  useEffect(() => {
    handleGetTopStoreSelling()
    handleGetBottomStoreSelling()
  }, [stateDateRange])
  return (
    <Box
      sx={{
    ...
    >
      <Stack
        ...
        mb={2}
      >
        <Typography sx={{ fontWeight: '500' }}>Top Stores</Typography>
        <Stack
          direction="row"
          alignItems="center"
      ...
      <Grid container spacing={2}>
        <Grid xs={4}>
          <Typography>Top 5 Selling Stores</Typography>
          <Chart
            series={stateListTopData?.map((i) => i.values)}
            options={{
              labels: stateListTopData.map((i) => i.store),
              legend: {
                show: true,
                position: 'bottom',
              },
        ...
        <Grid xs={4}>
          <Typography>Bottom 5 Selling Stores</Typography>
          <Chart
            series={stateListBottomData?.map((i) => i.values)}
            options={{
              labels: stateListBottomData.map((i) => i.store),
              legend: {
                show: true,
                position: 'bottom',
              },
...
export default ListPieChartStore
